...
class Management{
	...
	public void summonthpay() throws limitException{
		double total=0;
		for(Employee another:work.values()){
			total+=another.monthPay();
		}		
		if(total>50000){
			throw new limitException(total);
		}
		else{
...
class limitException extends Exception{
	limitException(double total){
		super("Total salary exceed limit: "+total);
	}
}
